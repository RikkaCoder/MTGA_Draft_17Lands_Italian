...
class TierListWindow(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(top_bar, text=tr("tier.imported"), bootstyle="secondary").pack(
            side="left"
        )

        self.btn_delete = ttk.Button(
            top_bar,
            text=tr("tier.delete_selected"),
            bootstyle="danger-outline",
            command=self._delete_selected,
        )
        self.btn_delete.pack(side="right", padx=Theme.scaled_val((5, 0)))

        self.var_filter = tkinter.StringVar(value=tr("tier.all_sets"))
        ...
        ttk.Label(top_bar, text=tr("tier.filter"), bootstyle="secondary").pack(
        ...
        ttk.Label(
            form_frame,
            text=tr("tier.import_new"),
            font=Theme.scaled_font(9, "bold"),
            bootstyle="primary",
        ).pack(anchor="w", pady=Theme.scaled_val((0, 10)))
        ttk.Label(form_frame, text=tr("tier.url"), bootstyle="secondary").pack(
            anchor="w"
        )
        ...
        ttk.Label(
            form_frame, text=tr("tier.custom_label"), bootstyle="secondary"
        ).pack(anchor="w")
        ...
        self.btn_import = ttk.Button(
            form_frame, text=tr("tier.download"), command=self._start_import
        )
    ...
    def _update_history_table(self):
        ...
        all_sets = tr("tier.all_sets")
        self.combo_filter["values"] = [all_sets] + sets
        if current_filter not in [all_sets] + sets:
            self.var_filter.set(all_sets)
            current_filter = all_sets

        filtered_files = []
        for f_set, f_label, f_date, f_name, _ in all_files:
            if current_filter != all_sets and f_set != current_filter:
                continue
    ...
    def _start_import(self):
        ...
        if not url.startswith(TIER_URL_17LANDS):
            messagebox.showwarning(tr("tier.invalid_url"), tr("tier.use_url"))
            return
        if not label:
            messagebox.showwarning(tr("tier.missing_label"), tr("tier.provide_label"))
            return

        self.btn_import.configure(state="disabled")
        self.vars["status"].set(tr("tier.connecting"))
        ...
        try:
            ...
            if self.winfo_exists():
                if new_tl:
                    ...
                    self._safe_error(tr("sealed.api_error"))
    ...
    def _finalize_import(self):
        ...
        self.vars["label"].set("")
        self.vars["status"].set(tr("tier.success"))
        if self.on_update_callback:
            self.on_update_callback()

    def _handle_error(self, err):
        self.btn_import.configure(state="normal")
        self.vars["status"].set(tr("tier.failed"))
        messagebox.showerror(tr("tier.import_error"), err)
